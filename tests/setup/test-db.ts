...
export const dropAllTestObjects = async (pool: Pool): Promise<void> => {
  ...
  await pool.query(
    "DROP VIEW IF EXISTS vw_active_academic_context, vw_admin_dashboard_summary, vw_user_notification_summary, vw_notification_details, vw_active_announcements, vw_announcement_details, vw_user_inbox_summary, vw_message_details, vw_trip_student_event_details, vw_active_trip_live_status, vw_latest_trip_location, vw_trip_details, vw_active_student_bus_assignments, vw_route_stops, vw_student_behavior_summary, vw_behavior_details, vw_homework_submission_details, vw_homework_details, vw_student_assessment_summary, vw_student_assessment_details, vw_assessment_details, vw_class_attendance_summary, vw_student_attendance_summary, vw_attendance_details, vw_class_students, vw_student_primary_parent, vw_student_profiles CASCADE;"
  );
  await pool.query(
    "DROP TABLE IF EXISTS analytics_scheduler_runs, analytics_feedback, analytics_snapshots, analytics_jobs, user_device_subscriptions, user_devices, integration_outbox, system_setting_audit_logs, system_settings, school_onboarding_import_runs, notifications, announcement_target_roles, announcements, messages, trip_student_events, transport_trip_eta_stop_snapshots, transport_trip_eta_snapshots, transport_route_map_cache, bus_location_history, trips, transport_route_assignments, student_transport_home_locations, student_bus_assignments, bus_stops, routes, buses, behavior_records, behavior_categories, homework_submissions, homework, student_assessments, assessments, assessment_types, attendance, attendance_sessions, student_promotions, student_parents, student_academic_enrollments, students, auth_refresh_tokens, password_reset_tokens, teacher_classes, supervisor_classes, subject_offerings, subjects, classes, semesters, grade_levels, academic_years, drivers, supervisors, teachers, parents, users, pgmigrations CASCADE;"
  );
};
